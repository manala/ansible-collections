# Ansible Collection - manala.utils
...
```shell
ansible-galaxy collection install manala.utils
```
...
See the [changelog](https://github.com/manala/ansible-utils/blob/main/utils/CHANGELOG.md).
...
ansible-test sanity --python 3.13 plugins/filter/default.py
```

Launch units tests over a specific file or not
```shell
ansible-test units --python 3.13 --venv
ansible-test units --python 3.13 --venv tests/unit/plugins/filter/test_default.py
```

Launch integration tests over a specific target or not
```shell
ansible-test integration
ansible-test integration default
ansible-test integration default --tags foo
```
